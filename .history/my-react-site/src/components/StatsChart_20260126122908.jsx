          ...
          <svg className="line-chart-svg" viewBox={`0 0 ${chartWidth} ${chartHeight}`} preserveAspectRatio="xMidYMid meet">
            ...
            {points.map((point, index) => (
              <g key={index}>
                ...
                {hoveredPoint === index && (
                  <g>
                    {/* Calculate tooltip position - flip if near top or edges */}
                    {(() => {
                      const tooltipWidth = 130;
                      const tooltipHeight = 65;
                      const flipVertical = point.y < 100;
                      const flipHorizontal = point.x > chartWidth - 100;
                      const flipLeft = point.x < 100;
                      
                      let tooltipX = point.x - tooltipWidth / 2;
                      let tooltipY = flipVertical ? point.y + 15 : point.y - tooltipHeight - 15;
                      
                      if (flipLeft) tooltipX = point.x + 15;
                      if (flipHorizontal) tooltipX = point.x - tooltipWidth - 15;
